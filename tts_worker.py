# در ابتدای فایل، قبل از هر import دیگری:
import sys
import logging
# لاگ‌گیری را در همان ابتدای کار تنظیم می‌کنیم
logging.basicConfig(
    level=logging.DEBUG, # سطح لاگ را به DEBUG تغییر می‌دهیم تا همه چیز ثبت شود
    format='[%(asctime)s] - [Python Worker] - %(levelname)s - %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S',
    stream=sys.stdout # همه لاگ‌ها را به stdout بفرست
)

# یک try-except بزرگ دور همه چیز قرار می‌دهیم
try:
    logging.debug("Starting tts_worker.py execution.")
    
    import base64
    logging.debug("Imported base64.")
    import mimetypes
    logging.debug("Imported mimetypes.")
    import os
    logging.debug("Imported os.")
    import re
    logging.debug("Imported re.")
    import struct
    logging.debug("Imported struct.")
    import time
    logging.debug("Imported time.")
    import uuid
    logging.debug("Imported uuid.")
    import shutil
    logging.debug("Imported shutil.")
    import json
    logging.debug("Imported json.")
    import threading
    logging.debug("Imported threading.")

    # تلاش برای import کردن کتابخانه‌های اصلی
    logging.debug("Attempting to import google.generativeai...")
    import google.generativeai as genai
    from google.generativeai import types
    GOOGLE_API_AVAILABLE = True
    logging.debug("Successfully imported google.generativeai.")

    logging.debug("Attempting to import pydub...")
    from pydub import AudioSegment
    PYDUB_AVAILABLE = True
    logging.debug("Successfully imported pydub.")
    ...
    def _init_api_keys():
        global ALL_API_KEYS
        logging.debug("Initializing API keys...")
    ...
    # ... (کدهای توابع save_binary_file, convert_to_wav, parse_audio_mime_type, smart_text_split, merge_audio_files_func اینجا قرار می‌گیرند) ...
    ...
    def generate_audio_chunk_with_retry(chunk_text, prompt_text, voice, temp, session_id):
        # ... (کد این تابع از پاسخ قبلی بدون تغییر) ...
        if not ALL_API_KEYS:
    ...
    def main():
        ...
        logging.debug(f"Received from stdin: {input_str[:200]}") # لاگ کردن بخشی از ورودی
        
        try:
            input_data = json.loads(input_str)
        except json.JSONDecodeError as e:
            logging.error(f"JSONDecodeError: {e}. Input was: {input_str}")
            sys.stdout.write(json.dumps({"success": False, "error": "ورودی JSON نامعتبر است."}))
            sys.exit(1)
            
        # ... (بقیه کد تابع main از پاسخ قبلی بدون تغییر) ...
    ...
    if __name__ == "__main__":
        logging.debug("__main__ block reached. Starting main function.")
        main()
        logging.debug("main() function finished.")

except Exception as e:
    # این بلاک هر خطایی که حتی قبل از شروع main رخ دهد را می‌گیرد.
    logging.critical(f"FATAL ERROR at top level: {e}", exc_info=True)
    # تلاش برای ارسال خطا به Node.js
    import json
    sys.stdout.write(json.dumps({"success": False, "error": f"خطای بحرانی در اسکریپت پایتون: {e}"}))
    sys.exit(1)
